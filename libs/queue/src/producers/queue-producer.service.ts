...
export class QueueProducerService implements IQueueProducer {
  ...
  async addJob(job: IQueueJob): Promise<string> {
    try {
      ...
      // Update metrics
      await this.metricsService.incrementJobCount(this.getQueueName(job.type), 'produced');
  ...
  async addJobs(jobs: IQueueJob[]): Promise<string[]> {
    ...
    try {
      ...
      this.logger.error(`Failed to add multiple jobs to queue: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async scheduleJob(job: IQueueJob, delay: number): Promise<string> {
    try {
      ...
      // Update metrics
      await this.metricsService.incrementJobCount(this.getQueueName(job.type), 'scheduled');

      this.logger.log(`Job ${job.id} scheduled for execution at ${scheduledAt.toISOString()}`);
      return job.id;
    } catch (error) {
  ...
  async cancelJob(jobId: string): Promise<boolean> {
    try {
      ...
      this.logger.warn(`Job cancellation not fully implemented for job: ${jobId}`);
      return false;
    } catch (error) {
  ...
  async getJobStatus(jobId: string): Promise<QueueJobStatus | null> {
    try {
      ...
      this.logger.warn(`Job status tracking not fully implemented for job: ${jobId}`);
      return null;
    } catch (error) {

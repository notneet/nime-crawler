...
export class InitTables1749050264134 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`sources\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(100) NOT NULL, \`slug\` varchar(50) NOT NULL, \`base_url\` text NOT NULL, \`selectors\` json NULL, \`headers\` json NULL, \`priority\` smallint NOT NULL DEFAULT '1', \`is_active\` tinyint NOT NULL DEFAULT 1, \`delay_ms\` int NOT NULL DEFAULT '5000', \`last_crawled_at\` timestamp NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_9a5cea873a16f3b91692a889d1\` (\`slug\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
